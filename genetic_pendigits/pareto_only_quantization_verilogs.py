...
def generate(identity,relusize,weight_size1,bias_size1,weight_size2,bias_size2,inputsize,relu_int,w1_int,b1_int,w2_int,b2_int,layer1,layer2,layer3,lrate,nepochs,X_train,Y_train,X_test,Y_test,pretrained1,pretrained2):
    ...
    weight_bias_size=[ [ (weight_size_f1,w1_int+1), (bias_size_f1,b1_int+1) ], [ (weight_size_f2,w2_int+1), (bias_size_f2,b2_int+1) ] ]
    relu_size=(relusize_f,reluint)

    model = Sequential()
    model.add(QDense(layer_2, input_shape=(layer_1,), name='fc1', kernel_quantizer=quantized_bits(weight_bias_size[0][0][0],w1_int,alpha=1,use_stochastic_rounding=True),bias_quantizer=quantized_bits(weight_bias_size[0][1][0],b1_int,alpha=1),
    ...
    model.add(QDense(layer_3, name='output',
                    kernel_quantizer=quantized_bits(weight_bias_size[1][0][0],w2_int,alpha=1,use_stochastic_rounding=True), bias_quantizer=quantized_bits(weight_bias_size[1][1][0],b2_int,alpha=1),
                    kernel_initializer='lecun_uniform', kernel_regularizer=l1(0.0001 ) ))
    ...
    callbacks= all_callbacks( outputDir = 'model_seeds_Paretos_only_quantization')
    model.fit(X_train, Y_train, batch_size=1,
            epochs=num_epochs,validation_split=0.2, verbose=0, shuffle=True,
            callbacks = callbacks.callbacks);
    model.compile(optimizer=adam, loss=['categorical_crossentropy'], metrics=['accuracy'])
    model_save_quantized_weights(model, "test_weights_only_quantization")
    ...
    filename = "top_pareto"+str(identity)+"_"+str(relusize_f)+str(weight_size_f1)+str(bias_size_f1)+str(weight_size_f2)+str(bias_size_f2)+str(input_s)+str(reluint)+"-"+str(accuracy[1])+str("Q")
    x = os.path.join("./only_quantization", filename)
    f=open(x,"w")
