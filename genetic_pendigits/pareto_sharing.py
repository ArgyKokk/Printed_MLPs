...
def top_sharing(identity, weights, bits_l1, bits_int_l1, bits_l2, bits_int_l2, bias_l1, bias_int_l1 , bias_l2 , bias_int_l2, inputsize, relu, relu_int, X_test, Y_test, layer1, layer2, layer3, clusters1, clusters2, window):
    ...
    filename = "Kmeans_"+str(identity)+"_w"+str(window)+"_"+str(clusters1 )+"C-"+str(clusters2)+"C-"+str(relusize_f)+str(weight_size_f1)+str(bias_size_f1)+str(weight_size_f2)+str(bias_size_f2)+str(input_s)+ str(int_relu)+"-"+str(new_accuracy[1])
    x = os.path.join("./pareto_custom_clustering", filename)
    f=open(x,"w")

    wv1.write_mlp_verilog(f, input_size, bias_list, weight_list, weight_bias_size, sum_relu_size,last_layer)
    f.close()

    filename = "sim.Xtest"+str(identity)
    x = os.path.join("./pareto_custom_clustering/datasets", filename)
    f=open(x,"w")
