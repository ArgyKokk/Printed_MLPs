...
def generate(model,identity,relusize,weight_size1,bias_size1,weight_size2,bias_size2,inputsize, int_relu, layer1,layer2,layer3,X_train,Y_train,X_test,Y_test):

    #1) extract the solution's parameters
    relusize_f = 8#int(relusize)
    weight_size_f1 =7# int(weight_size1)
    bias_size_f1 = 7#int(bias_size1)
    weight_size_f2 = 7#int(weight_size2)
    bias_size_f2 = 7#int(bias_size2)
    input_s = 4#int(inputsize)
    relusize_int = 1#int(int_relu)
    norm = 2**input_s
    layer_1 = layer1
    layer_2 = layer2
    layer_3 = layer3
    ...
    for i in range(2,max_clusters):
        ...
        model_clustered_quantized.add(QDense(layer_2, input_shape=(layer_1,), name='fc1', kernel_quantizer=quantized_bits(weight_bias_size[0][0][0],0,alpha=1,use_stochastic_rounding=True),bias_quantizer=quantized_bits(weight_bias_size[0][1][0],0,alpha=1),
