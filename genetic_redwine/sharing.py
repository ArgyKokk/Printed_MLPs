...
def top_sharing(weights, bits_l1, bits_int_l1, bits_l2, bits_int_l2, bias_l1, bias_int_l1 , bias_l2 , bias_int_l2, inputsize, relu, relu_int, X_test, Y_test, layer1, layer2, layer3, clusters1, clusters2, window):
    ...
    for node in range(0,1):
        ...
        #duplicates in goldenlist1
        for i in model1.trainable_variables[0]:
            temp=[]
            for j in i:
                temp.append(tf.get_static_value(j))
            for item, count in collections.Counter(temp).items():
                if count > 1:
                    if item not in GoldenList1:
                        GoldenList1.append(item)

        #duplicates in goldenlist2
        for i in model1.trainable_variables[2]:
            temp=[]
            for j in i:
                temp.append(tf.get_static_value(j))
            for item, count in collections.Counter(temp).items():
                if count > 1:
                    if item not in GoldenList2:
                        GoldenList2.append(item)
