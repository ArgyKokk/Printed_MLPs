...
def blackbox(wb1,wb2,relusize,weight_size1, bias_size1, weight_size2, bias_size2, sparsity, inputsize,int_relusize , int_w1, int_b1, int_w2, int_b2,layer_1,layer_2,layer_3,X_test,Y_test,X_train,Y_train):
    ...
    weight_bias_size=[ [ (weight_size_f1,intw1), (bias_size_f1,intb1) ], [ (weight_size_f2,intw2), (bias_size_f2,intb2) ] ]
    relu_size=(relusize_f,int_relu)

    model.add(QDense(layer_2, input_shape=(layer_1,), name='fc1', kernel_quantizer=quantized_bits(weight_bias_size[0][0][0],weight_bias_size[0][0][1],alpha=1,use_stochastic_rounding=True),bias_quantizer=quantized_bits(weight_bias_size[0][1][0],weight_bias_size[0][1][1],alpha=1),
    ...
    model.add(QDense(layer_3, name='output',
                    kernel_quantizer=quantized_bits(weight_bias_size[1][0][0],weight_bias_size[1][0][1],alpha=1,use_stochastic_rounding=True), bias_quantizer=quantized_bits(weight_bias_size[1][1][0],weight_bias_size[1][1][1],alpha=1),
                    kernel_initializer='lecun_uniform', kernel_regularizer=l1(0.0001 ) ))
    ...
    model.fit(X_train, Y_train, batch_size=1,
              epochs=4,validation_split=0.2, verbose=0, shuffle=True,
